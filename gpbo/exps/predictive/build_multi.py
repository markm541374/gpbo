...
def getVopt(optstate,persist,para,ev,aux, init=False):
    ...
    if init==False:
        ...
        if para['priorshape']=='gamma':
            L = np.empty([D,200])
            for i in range(D):
                L[i,:] = stats.gamma.rvs(para['mprior'][i+1], size=200, scale=para['sprior'][i+1], loc=0.)
            l_ = np.power(np.prod(L,axis=0),1./float(D))
        else:
            raise
    ind = np.argsort(l_)
    ...
    if debugoutput['predictive']:
        ...
        z = np.linspace(0.25*L.min(),1.5*L.max(),150)
...
def getVopt2(optstate,persist,para,ev,aux, init=False):
    ...
    if debugoutput['predictive']:
        ...
        a[4,0].hist(L[0,:],20,normed=1)
        dist = gammadist(*gammadist.fit(L[0,:],floc=0))
        a[4,0].twinx().semilogy(z,dist.pdf(z),'r')
        a[4,1].hist(L[1,:],20,normed=1)
        dist = gammadist(*gammadist.fit(L[1,:],floc=0))
        a[4,1].twinx().semilogy(z,dist.pdf(z),'r')

        a[0,0].hist(l,20,normed=1)

        dist = gammadist(*gammadist.fit(l,floc=0))
        a[0,0].twinx().semilogy(z,dist.pdf(z),'r')
        a[0,1].hist(A,20,normed=1)
        a[0,1].set_ylabel('A')
        a[0,0].set_ylabel('l')
