...
def f(x, **ev):
    ...
    rp = [6.86, 2000]
    rd = [1e-3,1000]
    p = rp[0] +(x[0]*0.5+0.5)*(rp[1]-rp[0])
    d = rd[0] +(x[1]*0.5+0.5)*(rd[1]-rd[0])
    ri = [1e-3,(p*d-d*6.68)/(0.5*0.3)]
    i = ri[0] +(x[2]*0.5+0.5)*(ri[1]-ri[0])
    rpx = [10.,5000.]
    px = rpx[0]+(rpx[1]-rpx[0])*(x[3]*0.5+0.5)
    rix = [1.,1000.]
    ix = rix[0]+(rix[1]-rix[0])*(x[4]*0.5+0.5)
    rdx = [0.1,100.]
    dx = rdx[0]+(rdx[1]-rdx[0])*(x[5]*0.5+0.5)
