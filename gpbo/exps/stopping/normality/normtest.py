...
def globallocalregret(optstate,persist,**para):
    ...
    JB = sp.stats.jarque_bera(Yin)
    logger.critical('JB = {}'.format(JB))
    logger.critical('JB2k = {}'.format(sp.stats.jarque_bera(Yin[:2000])))
    logger.critical('JBref = {}'.format(sp.stats.jarque_bera(np.random.normal(sp.mean(Yin),np.sqrt(sp.var(Yin)),len(Yin)))))
    print(len(Yin),np.array(Yin).max(),np.array(Yin).min(),sp.mean(Yin),sp.var(Yin),sp.stats.skew(Yin),sp.stats.kurtosis(Yin))
    return [JB[0],JB[1]]
    #f,a = plt.subplots(1)
    #a.hist(np.array(Yin),normed=1,bins=40)
    #plt.show()
...
def aftern(optstate,persist,**para):
    if optstate.n>para['n']:
        ...
        return 0,None,dict()
ojflist = ['branin','camel3','camel6','hart3','hart4','hart6']
Dims = [2,2,2]
#ojflist = ['hart3','hart4','hart6']
#Dims = [3,4,6]
for k in range(len(ojflist)):
    for n in range(16):
        D=Dims[k]
        file = '../'+ojflist[k]+'/results/pesfs_-4_{}.csv'.format(n)
        data= gpbo.optimize.readoptdata(file)
        C=gpbo.core.config.switchdefault(f,D,10,180,1e-6,'results','null.csv')
        O = gpbo.core.optstate()
        JB=[]
        with open('pes4/'+ojflist[k]+'_pesfs4_'+str(n)+'.csv','w') as f:
            for i in range(data.shape[0]):
                O.condition=-16
                O.update(np.array([data['x{}'.format(j)][i] for j in range(D)]),{'d':[sp.NaN],'s':0.},data['y'][i],1.,1.)
                while True:
                    try:
                        JB.append(globallocalregret(O,None,**C.choosepara))
                        break
                    except:
                        O.condition+=1
                        logger.critical('condition raised to {}'.format(O.condition))

                f.write(str(i)+', '+str(JB[-1][0])+', '+str(JB[-1][1])+'\n')
                f.flush()
