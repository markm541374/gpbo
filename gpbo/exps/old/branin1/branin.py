...
if run[0]:
    for k in xrange(nopts):
        C=gpbo.core.config.pesvsdefault(f,cfn,D,1,lsl,lsu,'results','pesvs{}.csv'.format(k+startfrom))
...
if run[2]:
    for k in xrange(nopts):
        C = gpbo.core.config.pesfslearns(f, D, 1, 10**lsu, 'results', 'pesfshigh{}.csv'.format(k+startfrom))
        C.stopfn = gpbo.core.optimize.cstopfn
        C.stoppara = {'cmax': cmax}
        out = gpbo.search(C)
if plot:
