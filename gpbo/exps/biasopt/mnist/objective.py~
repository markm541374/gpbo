...
def objective_function(x, s):
    print('started mnist with x={} s={}'.format(x,s))
    sys.stdout.flush()
...
def f(x, **ev):
    ...
    s = ev['xa']
    sn = int(np.exp(np.log(s_min) + (1.-s)*(np.log(s_max)-np.log(s_min))))
    u = x[0] *10.
    v = x[1] *10.

    f,c = objective_function(np.array([u,v]),sn)

    print( 'f inputs x:{} ev:{} (sn={}) outputs y:{}  c:{}'.format(x, ev, sn, f, c))
    sys.stdout.flush()
    return f, c, dict()
